...
{% highlight shell %}CREATE DATABASE tipi;
...
{% highlight shell %}$ sudo apt-get install tomcat7
$ firefox localhost:8080
{% endhighlight %}

Get TiPis WAR-file if you haven't deployed it by yourself with guide after this.

{% highlight shell %}$ wget https://download.soivi.net/TiPi.tar.xz
$ tar -vxf TiPi.tar.xz
...
{% highlight shell %}$ firefox localhost:8080/TiPi
{% endhighlight %}

Here are users Password to signing in

{% highlight shell %}Test User
...
{% highlight shell %}db.driver=com.mysql.jdbc.Driver
...
{% highlight shell %}$ nano log4j.properties
{% endhighlight %}

Add these lines to your logging configuration file

{% highlight shell %}# Set root logger level to DEBUG and its only appender to A1.
...
{% highlight shell %}$ cp /usr/share/tomcat7/lib/servlet-api.jar ~/TiPi/WebContent/WEB-INF/lib/
{% endhighlight %}
...
{% highlight shell %}$ tar -xvzf eclipse-jee-mars-2-linux-gtk-x86_64.tar.gz
{% endhighlight %}
...
{% highlight shell %}$ cp -r ~/TiPi/* ~/workspace/TiPi/
{% endhighlight %}
...
{% highlight shell %}<classpathentry kind="src" path="conf"/>
{% endhighlight %}

Configure .classpath

{% highlight shell %}$ nano ~/workspace/TiPi/.classpath
{% endhighlight %}
...
{% highlight shell %}$ sudo mv ~/TiPi.war /var/lib/tomcat7/webapps/
...
{% highlight shell %}$ sudo rm /var/lib/tomcat7/webapps/TiPi.war
{% endhighlight %}
